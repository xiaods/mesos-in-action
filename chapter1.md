...
当前云计算已经成为企业 IT 部门采用的最常用的。云计算平台是由多台虚拟主机构成的集群管理系统，通过分散在各地数据中心的虚拟计算资源来支撑企业的业务计算服务。为了提高计算资源的利用率，全球的开发者已经开发出来各种各样优秀的计算框架来支撑复杂的数据计算业务，典型的包括MapReduce、Kafka、Spark等等。虽然每个一段时间业界就会出来新的计算框架，但我们仍然没有找到一个全能框架能完全满足全部的企业应用需求。
