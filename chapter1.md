...
当前云计算服务已经深入到企业应用的方方面面。云计算平台是由多台虚拟主机构成的集群管理系统，通过分散在各地数据中心的虚拟计算资源来支撑企业的业务计算服务。为了提高计算资源的利用率，全球的开发者已经开发出来各种各样优秀的计算框架来支撑复杂的数据计算业务，典型的包括MapReduce、Kafka、Spark等等。虽然每个一段时间业界就会出来新的计算框架，但我们仍然没有找到一个全能框架能完全满足全部的企业应用需求。

Mesos，一个 Apache 开源项目，致力于构建数据中心级别的集群管理系统。它通过虚拟化物理主机（或者虚拟主机）之上的 CPU、内存、磁盘以及其它计算资源，构建高容错的、可动态伸缩的分布式系统，让运行计算任务快捷有效。Mesos 帮助开发者可以在一个统一的资源池之上运行应用，简化了构建分布式系统的复杂度。
