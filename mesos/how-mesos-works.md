...
本节将简单介绍下 Mesos 是怎样工作的，以及通过简单的实例来展示 Mesos 的魔力。
更多更具深度，难度的真实用例将在本书其它章节详细介绍。

## Why Mesos

简单回顾下计算机界的发展历程，可以发现一些规律，例如：CPU 的发展是单核 CPU
主频越来越高，计算能力越来越强，然后当单核计算能力越来越难于提高时，多核 CPU
迎难而上，继续提高着单个计算结点的计算能力，但是当单个计算结点计算能力越来越难于提高，
或者说成本越来越高时，分布式计算迎难而上。

而当各种分布式计算框架越来越流行时，问题出现了，各个框架独自占有各自的计算集群，
即使在任务不多或者没有任务时，资源依然不能被其它框架共享。究其原因，大概主要有以下几方面。

### 集群搭建复杂，配置管理难度大

集群搭建复杂，配置难度大，主要在于如果将多个计算框架部署在一个集群上，
一旦集群结点达到几十甚至上百个，那么配置管理的难度将非常大，并且指数上升。

### 硬件可能定制化

由于各个计算框架都是针对一定使用场景设计的，所以其任务的类型也具有一定模式，例如：
当前流行的 Hadoop, Spark, Storm 等计算框架，各自都有自己擅长的计算类型；相应地，
资源需求类型也不同：Hadoop 需要高性能的磁盘 I/O 以便加速 MapReduce 读写的速度，
Spark 需要大容量内存因为其将计算中间结果都存放在内存中，而 Storm
则没有类似特要求。

所以，为了更加节省计算资源，充分利用资源，硬件可能定制化，例如：给 Hadoop
集群配备 SSD，给 Spark 集群配置大容量内存等。

所以，定制化的硬件并不能很好的适用于其它计算框架。
...
在任务隔离上，基于 Linux Controll Groups, Mesos 可以做到多方面的隔离，
...
从上面的日中中，我们可以知道，在 A 机器上，mesos-slave 表明自己可用的资源为
...
这是 mesos-slave 自动检测的结果，如果有需要，我们可以手动配置，例如：
在我们上一节的实例中，我们可以指定一半的资源给 mesos-slave 使用，那么就可以保证
mesos-master 有足够的资源运行。
...
分别对应 CPU，内存，磁盘和网络端口。另外，CPU
和内存是必须资源，缺少其中任何一种都不能运行任务。例如：可以指定 A
机器的资源为：
...
注意：修改资源配置只要在重启 mesos-slave 后才生效。
...
上面的日志显示 mesos-master 向框架 FIXME 发送了 2 个 Offer。那么，这两个 Offer
是什么样的呢？我们可以到框架的日志中查看，例如：

```
FIXME: log output
```

可以看到，一个 Offer 就是一个计算结点在某一时刻对当前框架的所有可用资源。
这里，需要注意一些限定：
